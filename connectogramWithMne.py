...
exprt_mat = pd.read_excel(r'ouputresults_experimental.xlsx',header=None).to_numpy()
...
con = np.where((control_mat<=0.05) & (control_mat>0) ,control_mat,np.nan)
...
white= mpl.colormaps['binary']
fig, axes = plot_connectivity_circle(con,node_names,node_angles=node_angles,colorbar=True, node_colors= ['deepskyblue'],colormap=ListedColormap(white(np.linspace(0.0, 0.01, 1)))
,
title='Number Of Fibers \n\n         Left Hemisphere               Right Hemisphere     ', facecolor='white',textcolor='black',linewidth=3)

#ListedColormap(new_blues(np.linspace(0.01, 0.75, 128)))